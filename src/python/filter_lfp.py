...
def get_recording_path(base_path, animal, date, session):
...
def get_valid_times(recording, atol=1e-6):
    ...
    valid_times = [
        (timestamps[i], timestamps[jump_times[i + 1]])
...
def filter_data(recording, filter_coeff, valid_times, decimation=None, channels=None):
    ...
    timestamps = dask.delayed(recording.get_times())
    n_samples = recording.get_num_samples()
    ram_capacity = psutil.virtual_memory().available / (1024**3) * 0.9
    rec_disk_mem = recording.get_memory_size() / (1024**3)
    data_on_disk = dask.delayed(recording.get_traces(channel_ids=channels))
    ...
    if rec_disk_mem > ram_capacity:
        print("Recording size exceeds available RAM. Loading recording in chunks.")
        for start, stop in valid_times:
            ...
            shape, _ = gsp.filter_data_fir(
                ...
                describe_dims=True,
                ds=decimation,
                input_dim_restrictions=[None, np.s_[channels]],
            )
            output_offsets.append(output_offsets[-1] + shape[0])
            output_shape_list[0] += shape[0]
        filtered_data = np.empty(
            tuple(output_shape_list), dtype=data_on_disk.dtype.compute()
        )
        results = []
        new_timestamps = np.empty((output_shape_list[0],), timestamps.dtype.compute())
        indices = np.array(indices, ndmin=2)
        ts_offset = 0
        for i, (start, end) in enumerate(valid_times):
            ...
            results.append(
                dask.delayed(gsp.filter_data_fir)(
                    data_on_disk,
                    filter_coeff,
                    axis=0,
